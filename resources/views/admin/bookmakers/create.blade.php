<x-admin-layout title="Add Bookmaker">
    <x-slot name="breadcrumb">Bookmakers / Create</x-slot>
    <x-slot name="actions">
        <a href="{{ route('admin.bookmakers.index') }}" class="btn-secondary">← Back</a>
    </x-slot>

    <div style="max-width:760px">
        <form method="POST" action="{{ route('admin.bookmakers.store') }}">
            @csrf
            ...
            <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:1.4rem;margin-bottom:1rem">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                    ...
                    <div class="admin-form-group">
                        ...
                        <input type="url" name="affiliate_url" class="admin-input" value="{{ old('affiliate_url') }}" placeholder="https://" required>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                    <div class="admin-form-group">
                        ...
                        <input type="text" name="welcome_offer" class="admin-input" value="{{ old('welcome_offer') }}" placeholder="e.g. €100 Free Bet">
                    </div>
                    <div class="admin-form-group">
                        <label class="admin-label">Bonus Text (small print)</label>
                        <input type="text" name="bonus_text" class="admin-input" value="{{ old('bonus_text') }}" placeholder="18+ T&Cs apply">
                    </div>
                </div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                    <div class="admin-form-group">
                        ...
                        <input type="number" name="rating" class="admin-input" value="{{ old('rating', '4.0') }}" min="0" max="5" step="0.1" required>
                    </div>
                    <div class="admin-form-group">
                        <label class="admin-label">Sort Order</label>
                        <input type="number" name="sort_order" class="admin-input" value="{{ old('sort_order', 99) }}">
                    </div>
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Full Review</label>
                    <textarea name="review" class="admin-textarea" style="min-height:140px">{{ old('review') }}</textarea>
                </div>
            ...
            <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:1.4rem;margin-bottom:1rem">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:.5rem">
                    ...
                    <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;padding:.35rem .5rem;border-radius:4px;border:1px solid var(--border);font-size:.82rem;color:var(--text)">
                        <input type="checkbox" name="markets[]" value="{{ $market->id }}"
                               style="accent-color:var(--accent)"
                               {{ in_array($market->id, old('markets', [])) ? 'checked' : '' }}>
                        {{ $market->name }}
                    </label>
                    @endforeach
                </div>
            ...
            <div style="display:flex;gap:.75rem">
                <button type="submit" class="btn-primary">Save Bookmaker</button>
