<x-admin-layout title="Add Bet Market">
    <x-slot name="breadcrumb"><a href="{{ route('admin.bet-markets.index') }}" style="color:var(--accent);text-decoration:none">Bet Markets</a> / New</x-slot>

    <div style="max-width:560px">
        <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:1.5rem">
            <form method="POST" action="{{ route('admin.bet-markets.store') }}">
                @csrf

                <div class="admin-form-group">
                    <label class="admin-label">Name *</label>
                    <input type="text" name="name" class="admin-input" value="{{ old('name') }}" required placeholder="e.g. Match Result">
                    @error('name')<div style="color:#ef4444;font-size:.75rem;margin-top:.3rem">{{ $message }}</div>@enderror
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Slug *</label>
                    <input type="text" name="slug" class="admin-input" value="{{ old('slug') }}" required placeholder="e.g. match-result">
                    @error('slug')<div style="color:#ef4444;font-size:.75rem;margin-top:.3rem">{{ $message }}</div>@enderror
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Category</label>
                    <input type="text" name="category" class="admin-input" value="{{ old('category') }}" placeholder="e.g. Match, Goals, Cards">
                    @error('category')<div style="color:#ef4444;font-size:.75rem;margin-top:.3rem">{{ $message }}</div>@enderror
                </div>

                <div class="admin-form-group">
                    <label class="admin-label">Description</label>
                    <textarea name="description" class="admin-textarea" rows="3" placeholder="Optional description...">{{ old('description') }}</textarea>
                ...
                <div style="display:flex;gap:.75rem;margin-top:1.25rem">
                    <button type="submit" class="btn-primary">Save Market</button>
